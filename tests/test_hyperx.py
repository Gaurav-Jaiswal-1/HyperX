...
def test_pytorch_hyperx():
    ...
    assert torch.all(torch.abs(result) <= torch.abs(x)) # Ensure HyperX reduces extreme values
...
def test_tensorflow_hyperx():
    ...
    result = activation(x)
    assert result.shape == x.shape
    assert tf.reduce_all(result <= x).numpy()
  # Ensure HyperX reduces extreme values
